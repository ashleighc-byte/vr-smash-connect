...
>>>>>>> ac0aed0 (Add playoff sign-up flow with all routes, pages, and migrations)
...
function SchoolBracketPage() {
  ...
  const { school: searchSchool, players: searchPlayers } = Route.useSearch();
  ...
  const publishBracketFn = useServerFn(async (data: { school: string; bracket_data: unknown }) => {
    ...
    const { error } = await supabaseAdmin
      ...
      .upsert({
        school: data.school,
        bracket_data: data.bracket_data,
        is_live: true,
  ...
  const publishBracket = useCallback(async () => {
    if (!confirm(`Publish this bracket for ${school}? It will be visible to students at /bracket/${school.toLowerCase().replace(/\s+/g, "-")}.`)) return;
    setPublishing(true);
    setPublishMsg(null);
    try {
      const bracketState = {
      ...
      await publishBracketFn({ data: { school, bracket_data: bracketState } });
      setPublishMsg("✓ Bracket published! Students can view it now.");
    } catch {
